...
- Building chat applications, live notifications, or real-time feeds. See [Serverless API Gateway](/recipes/nginx-reverse-proxy/) for HTTP endpoint patterns.
- Streaming live data to dashboards or monitoring tools. See [Event-Driven Functions](/recipes/event-driven-microservices/) for event-driven data streaming.
...
- Broadcasting events from backend services to connected clients. See [Scheduled Jobs](/recipes/background-jobs/) for periodic data push.
...
- **Broadcasting**: To send a message to all clients, scan the connections table and call `postToConnection` for each `connectionId`.
...
- **Scanning DynamoDB for large audiences**: a full table scan on thousands of connections is slow and expensive.
...
- **Broadcasting to all connections for every message**: not all messages need to reach all clients.
...
- **Related guides**: explore the serverless and websockets guides for deeper coverage.
...
- **Apply build real-time apis with websockets on serverless** when you need a practical solution for your use case.
