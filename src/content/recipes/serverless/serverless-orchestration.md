...
- A serverless process has more than two dependent steps. See [Serverless Functions](/recipes/event-driven-microservices/) for building individual function units.
- Steps must be retried with backoff on transient failures. See [Retry Logic](/recipes/retry-backoff/) for exponential backoff patterns.
- Workflow state must survive function timeouts or crashes
- Human approval or external event waits are required mid-workflow
- Multiple functions must be coordinated with parallel branches and join points. See [Event-Driven Functions](/recipes/event-driven-microservices/) for event-driven architectures.
...
- **Keep Lambda functions stateless and idempotent**: the orchestrator handles state; functions should be pure.  If a function is retried, it must produce the same result without side effects.
- **Use exponential backoff with jitter for retries**: retrying every 1 second creates thundering herds.  Add random jitter to spread retries.  Step Functions supports this natively; in Temporal, configure retry policies on activities.
...
- **Monitor workflow execution metrics**: track success rate, average duration, retry count per activity, and state transition cost.
...
- **Not versioning state machines**: changing a live state machine definition affects in-flight executions.

## Advanced: Saga Pattern with Compensation

For workflows that span multiple services, the saga pattern coordinates distributed transactions with compensating actions on failure:
...
- **Related guides**: explore the serverless and aws-lambda guides for deeper coverage.
...
- **Apply orchestrate serverless workflows with step functions and** when you need a practical solution for your use case.
...
### How do I implement the saga pattern with compensation?

Register a compensation function before each step in the workflow. If any step fails, run compensations in reverse order. In Temporal, use a compensations array and iterate in reverse on catch. In Step Functions, use `Catch` blocks that invoke compensation Lambda functions. Each compensation should be idempotent — it may run multiple times.

### How do I version workflows without breaking in-flight executions?

Step Functions: use versions and aliases to deploy new definitions without affecting running executions. Temporal: use `WorkflowVersion` to branch logic based on the version the execution started with. Durable Functions: use orchestration versioning with `IDurableOrchestrationContext`. Always test new versions with a canary percentage before full rollout.
