...
- Complete audit history of all changes is a business requirement. See [Event-Driven Functions](/recipes/event-driven-microservices/) for event-driven architectures.
- You need to answer temporal questions about past states
- Rebuilding read models from scratch is a needed capability. See [Serverless Orchestration](/recipes/background-jobs/) for managing stateful workflows.
- The write model is complex and the read model needs to be optimized separately
- Compliance or regulatory requirements mandate immutable change logs. See [CQRS Pattern](/patterns/cqrs-pattern/) for separating read and write models.
