...
- Authenticating backend services, [microservices](/guides/microservices-architecture-guide/), or serverless functions to each other
- Providing third-party developers access to a public API with usage limits
- Securing [webhook](/recipes/webhooks/) endpoints that receive push notifications from external providers
...
- **[Rate limiting](/recipes/api-rate-limiting-redis/) per key**: track request counts per API key in Redis with TTL windows.  Apply tiered limits — a free-tier key gets 100 requests/hour while an enterprise key gets 100,000.
...
- **Using predictable key formats**: sequential IDs or UUIDv1 keys leak generation time. token_urlsafe` or `/dev/urandom`).
- **Storing keys in client-side code**: mobile apps and frontend JavaScript cannot keep secrets.
...
- **Hardcoding keys in configuration**: storing production keys in `config. json` or environment variables on shared servers exposes them to all processes.
...
- **Apply secure api key authentication for services and clients** when you need a practical solution for your use case.
...
**Q: What is the difference between API keys and [JWT tokens](/recipes/jwt-authentication/)?**
