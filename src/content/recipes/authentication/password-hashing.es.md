...
- **Nunca inventes tu propia criptografía**: La criptografía es notoriamente fácil de hacer mal de formas sutiles que solo se hacen evidentes bajo ataque.
...
- **Nunca compares en texto plano**: Estas realizan comparación en tiempo constante para prevenir ataques de timing que podrían filtrar información sobre la contraseña.
...
- **Hard-codear salts en el código fuente**: Un salt hard-codeado es tan malo como no tener salt, ya que los atacantes lo encontrarán en el repositorio.
...
- **Almacenar el hash sin el identificador de algoritmo**: Almacena siempre el string de output completo de bcrypt/Argon2 que incluye el algoritmo, costo, salt y hash.
- **Enviar contraseñas sobre conexiones no encriptadas**: El hashing protege las contraseñas almacenadas, pero la contraseña debe viajar de forma segura a tu servidor primero.
...
- **Guías relacionadas**: explora las guías de authentication y bcrypt para profundizar.
...
- **Aplica hashing de contraseñas** cuando necesites una solución práctica para tu caso de uso.
...
- **Pepper storage compromise**: Almacena peppers en un secrets manager (Vault, AWS Secrets Manager).
...
- **Argon2 parameter tuning**: si los parametros de Argon2 (memory, iterations, parallelism) son demasiado bajos, el hash es vulnerable a GPU attacks.
- **Salt reuse across users**: si se usa el mismo salt para todas las passwords, se pueden precomputar rainbow tables para ese salt.
- **Hash timing side-channel**: Usa constant-time comparison functions.
- **Password storage in reversible form**: si las passwords se encriptan en lugar de hashear, un key compromise expone todas las passwords.
...
- **Memory-hard hash DoS**: si memory-hard hashes (Argon2, scrypt) se usan para cada login, atacantes pueden exhaustar server memory con muchos concurrent logins.
- **Hash format confusion**: si el sistema acepta multiples hash formats (bcrypt, scrypt, Argon2, PBKDF2), un bug de format parsing podria bypassar verificacion.
...
- **Hash comparison short-circuit**: si la hash comparison para en el primer byte que difiere, atacantes pueden medir timing para determinar hash prefixes.
- **Password pepper rotation**: si el pepper se rota, todos los hashes existentes se vuelven invalidos.
- **Hash storage in memory**: si los password hashes se cachean en memoria para performance, un memory dump los expone.
- **Weak random for salt generation**: random() en lugar de crypto.  randomBytes(), los salts son predecibles.
...
- **Concurrent hash computation race**: si dos concurrent logins para el mismo usuario computan hashes simultaneamente, una race condition puede causar verificacion incorrecta.
