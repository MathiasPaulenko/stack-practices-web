...
- You want to offer "Sign in with Google / GitHub" on your platform. See [Magic Links](/recipes/magic-link-authentication/) for passwordless alternatives.
...
**PKCE** (Proof Key for Code Exchange) adds a secret verifier to prevent interception attacks on mobile and SPA apps. **State** prevents [CSRF](/recipes/session-management/) by binding the callback to the original request.
...
- **Related guides**: explore the authentication and oauth2 guides for deeper coverage.
...
- **Apply oauth 2.0 login** when you need a practical solution for your use case.
...
Use a library that abstracts provider differences (Passport.js, Authlib, Spring Security). See [API Key Authentication](/recipes/api-key-authentication/) for machine-to-machine auth. Store provider-specific fields (`provider`, `provider_user_id`) in your user table. Normalize email/name fields across providers to create a unified user profile.
...
- **OAuth2 token leakage via referrer**: if OAuth2 callbacks redirect with tokens in the URL, the Referer header may leak tokens to third-party sites.
...
- **OAuth2 refresh token rotation**: if refresh tokens are not rotated on each use, a stolen token can be used indefinitely.
- **OAuth2 scope validation bypass**: if scope validation is case-insensitive, attackers can bypass scope checks by using different cases.
- **OAuth2 code verifier entropy**: if the PKCE code verifier is too short, attackers can brute force it.
- **OAuth2 redirect URI prefix matching**: if redirect URIs are matched by prefix, attackers can register https://evil. com/callback to match https://app. com/callback.
- **OAuth2 state parameter fixation**: if the state parameter is predictable, attackers can guess it and perform CSRF.
...
- **OAuth2 implicit flow token exposure**: the implicit flow returns access tokens in the URL fragment, which is accessible to any script on the page.
