...
- Two-phase commit (2PC) is unavailable or unacceptable due to latency or lock contention. See [Event-Driven Architecture](/recipes/event-driven-architecture/) for lock-free coordination.
- Long-running operations must survive temporary service unavailability
- Compensating actions are feasible (e.g., refund payment, release inventory, cancel shipment). See [Microservices Patterns](/guides/microservices-architecture-guide/) for resilience strategies.
