...
- [Database connections](/recipes/connection-pooling/) occasionally timeout under load
...
- **Use idempotency keys**: Retrying POST requests without them creates duplicates. See [message idempotency](/recipes/rabbitmq-task-queue/).
- **Circuit breaker integration**: Stop retrying when the service is clearly down. Integrate with [circuit breaker](/patterns/circuit-breaker-pattern/).
...
1. **Retrying everything**: [Non-idempotent](/recipes/rabbitmq-task-queue/) operations and client errors should fail fast
...
A: Generate a unique `Idempotency-Key` header. The server checks if it has processed this key before. Learn more in [message idempotency](/recipes/rabbitmq-task-queue/).
