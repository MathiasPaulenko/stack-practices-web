...
- **Implement [circuit breakers](/recipes/circuit-breaker-pattern/)**: Stop retrying if downstream is unhealthy
...
Score data quality per batch to track pipeline health. Define quality dimensions: completeness (non-null ratio), uniqueness (duplicate rate), validity (schema conformance), timeliness (freshness), and consistency (cross-field rules). Calculate per batch: `completeness = 1 - (null_count / total_count); uniqueness = unique_count / total_count; validity = valid_count / total_count`. Store scores: `CREATE TABLE data_quality_scores (id SERIAL PRIMARY KEY, job_id VARCHAR(64), batch_id INT, completeness FLOAT, uniqueness FLOAT, validity FLOAT, composite_score FLOAT, created_at TIMESTAMP DEFAULT NOW())`. Alert when composite score drops below 0.95. Use a weighted score: `composite = 0.3 * completeness + 0.3 * uniqueness + 0.2 * validity + 0.2 * timeliness`. Generate weekly trends: `SELECT date_trunc('week', created_at), AVG(composite_score) FROM data_quality_scores GROUP BY 1`. For anomaly detection, use z-scores: `z = (current_score - avg_score) / std_score` and alert when `z < -2`. Integrate with Apache Griffin or Deequ for automated quality scoring at scale.
