...
- [API responses](/recipes/call-rest-api/) are cached and must not be modified by consumers
...
- For React state, consider Immer for structural sharing instead of full cloning. See [Clean Code Guide](/guides/clean-code-principles-guide/) for maintainable patterns.

## Common Mistakes

- Using `JSON.parse` for objects containing Dates, Maps, or functions
- Spreading nested objects (`{ ...obj }`) which only shallow-clones the first level. See [Deep Clone JavaScript](/recipes/deep-clone-javascript/) for complete strategies.
