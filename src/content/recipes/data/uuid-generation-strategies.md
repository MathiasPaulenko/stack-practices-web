...
- [Database](/recipes/database-transactions/) primary keys need to be globally unique across distributed systems
...
- Use UUID v7 for new applications needing time-sortable keys. See [Database Migrations](/recipes/database-migrations/) for evolving schemas.
...
A: Use integers for single-node systems where coordination is trivial. Use UUIDs for distributed systems or when identifiers must not reveal sequence information. See [Database Connection Pooling](/recipes/database-connection-pooling/) for managing database connections.
