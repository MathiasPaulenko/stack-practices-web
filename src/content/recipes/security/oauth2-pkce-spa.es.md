...
- Valida siempre el **parametro state** para prevenir ataques [CSRF](/recipes/api-security-headers/)
- Usa **Content Security Policy** headers para mitigar robo de tokens por [XSS](/recipes/xss-prevention/)
...
- Almacenar tokens en `localStorage` donde [XSS](/recipes/xss-prevention/) puede robarlos facilmente
...
R: Usa un patron backend-for-frontend (BFF) donde tu [backend maneja el flujo OAuth](/recipes/oauth2-pkce-spa/) y la SPA se autentica via cookies de sesion.
