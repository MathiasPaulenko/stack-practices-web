...
SQL injection is one of the most common and dangerous web application vulnerabilities. It occurs when an attacker injects malicious SQL code into application queries through user input, potentially exposing, modifying, or deleting entire databases. Injection attacks consistently rank in the [OWASP Top 10](/guides/security-best-practices-guide/) because they are easy to exploit and devastating in impact.

The root cause is almost always the same: concatenating untrusted user [input](/recipes/input-validation/) directly into SQL strings. The fix is equally straightforward: use parameterized queries or an ORM that handles escaping automatically. The pattern below demonstrates the secure way to access databases in Python, JavaScript, and Java.
...
- **Validate input before it reaches the database**: [input validation](/recipes/input-validation/) and parameterized queries are complementary defenses.
...
A: [NoSQL injection](/guides/nosql-database-selection-guide/) exists too. Use parameterized queries or driver methods that accept objects, not string concatenation. Never pass raw user input to `eval()` or `$where` clauses.
