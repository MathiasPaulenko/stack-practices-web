...
- **Storing tokens in localStorage**: [XSS](/recipes/xss-prevention/) can steal localStorage. Store the server-side token in a hidden form field or a non-HttpOnly cookie (for double-submit pattern).
...
A: APIs that accept form submissions or use cookie authentication need CSRF protection. APIs that use bearer tokens or API keys in headers are generally immune because the attacker cannot forge the header. For API header security, see [API security headers](/recipes/api-security-headers/).
