...
- Escribiendo unit tests para código que llama bases de datos, APIs o servicios de terceros. Consulta [Integration Testing](/recipes/integration-testing/) para testear con dependencias reales.
- Testeando manejo de errores para escenarios difíciles de disparar en sistemas reales. Consulta [API Contract Testing](/recipes/api-mocking/) para verificar respuestas de error de API.
...
- Reemplazando dependencias no determinísticas (generadores random, hora actual, UUIDs). Consulta [Call REST API](/recipes/call-rest-api/) para testear lógica de clientes HTTP.
