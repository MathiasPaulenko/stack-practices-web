...
Los fixtures de test son los datasets y estados de entorno conocidos y controlados que hacen los tests deterministas. Sin fixtures, los tests dependen de bases de datos externas, sistemas de archivos o estado aleatorio, produciendo fallos flaky que desperdician tiempo de debugging. La solucion abajo muestra cómo crear, aislar y limpiar fixtures usando patrones factory y hooks nativos del framework. Recursos relacionados: [Medir y Exigir Cobertura de Tests con pytest-cov](/recipes/python-coverage-pytest-cov).

## Cuándo Usar


- For alternatives, see [Pytest Fixtures and Parametrize](/es/recipes/python-pytest-fixtures-parametrize/).
...
A: Un fixture proporciona el entorno o datos de prueba (por ejemplo, una base de datos sembrada). Un mock reemplaza una dependencia real para aislar el código bajo prueba.

**Q: ¿Cuándo debo limpiar los fixtures después de un test?**
A: Siempre, a menos que compartas intencionalmente un setup costoso dentro de una clase de tests de alcance reducido. La limpieza previene la contaminación entre tests y resultados flaky.

**Q: ¿Cómo comparto fixtures entre archivos de test?**
A: Usa fixtures a nivel de framework (pytest conftest, Jest setupFiles) o un módulo helper compartido. Mantén los fixtures compartidos mínimos y deterministas.
