...
- **Replace in-memory store with Redis** for multi-instance deployments. See [API Rate Limiting with Redis](/recipes/api-rate-limiting-redis/) for Redis patterns.
- **Add rate limiting** to prevent API cost surprises. See [API Rate Limiting with Redis](/recipes/api-rate-limiting-redis/) for implementation.
- **Implement function calling** to let the bot execute actions. See [AI Agents with Tool Use](/recipes/ai-agents-tool-use/) for function calling patterns.
