...
- Designing [chatbots](/recipes/chatbot-openai/), copilots, or [AI-powered assistants](/recipes/ai-agents/)
- Implementing automated content moderation, summarization, or translation pipelines
- Evaluating prompt versions with [A/B testing](/recipes/load-testing-k6/) frameworks
...
- **Related guides**: explore the ai and machine-learning guides for deeper coverage.
...
- **Apply apply prompt engineering: what works** when you need a practical solution for your use case.
...
A: No. [Prompt engineering](/recipes/prompt-engineering/) is faster to iterate and requires no data preparation. [Fine-tuning](/recipes/llm-fine-tuning/) is better when you need consistent behavior on a specialized domain and want to reduce per-request token costs.

**Q: Can I force an LLM to always output valid JSON?**
A: OpenAI's `json_object` response format and [function calling](/recipes/ai-agents-tool-use/) enforce JSON structure, but the model can still produce semantically incorrect or hallucinated values. Validate schema server-side.
