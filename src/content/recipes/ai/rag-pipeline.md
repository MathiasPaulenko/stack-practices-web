...
Retrieval-Augmented Generation (RAG) combines a large language model with a document retrieval step. Instead of relying solely on the LLM's parametric memory, [RAG](/recipes/rag-pipeline/) fetches relevant passages from a knowledge base and injects them into the prompt. This dramatically reduces hallucinations and enables the model to answer questions about private or recent data.
...
1. **Ingestion**: Documents are loaded, cleaned, and split into chunks. For document handling, see [file upload validation](/recipes/file-upload-validation/). Chunk size (typically 200–1000 tokens) balances granularity with context preservation.
2. **Embedding**: Each chunk is converted into a high-dimensional vector using an embedding model. Vectors capture [semantic meaning](/recipes/semantic-search/), not just keyword overlap.
...
|------------|----------|-------|
| LangChain | Orchestration + chaining | Mature ecosystem, handles [prompt templates](/recipes/prompt-engineering/) and output parsing |
...
- **Related guides**: explore the ai and embeddings guides for deeper coverage.
...
- **Apply build a rag pipeline with langchain and vector databases** when you need a practical solution for your use case.
