...
Las dead letter queues (DLQs) capturan mensajes que fallan el procesamiento después de intentos repetidos. Sin ellas, los mensajes fallidos bloquearían la cola o se perderían por completo. Un sistema DLQ bien diseñado distingue entre poison pills (mensajes permanentemente malos) y fallas transitorias, habilitando a operadores a replayear, inspeccionar o descartar mensajes problemáticos sin impactar el flujo principal de procesamiento.
...
- Compliance requiere audit trails de todos los mensajes procesados y fallidos
...
- **Correlación**: Vincular mensaje de DLQ al trace ID original
...
- **Monitorea profundidad de DLQ como métrica**: Es un indicador leading de salud del sistema
...
2. **Loops infinitos de retry**: Requeuear sin un count máximo crea procesamiento perpetuo
...
R: Las colas de retry retienen mensajes para reprocesamiento posterior. Las DLQs retienen mensajes que agotaron todos los retries.
