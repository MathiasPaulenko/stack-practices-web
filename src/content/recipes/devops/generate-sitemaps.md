...
- Your site has thousands of pages that change regularly and a static sitemap is unmaintainable. See [Background Jobs](/recipes/background-jobs/) for scheduled regeneration.
- You run a multi-language site and need `xhtml:link` annotations in sitemaps for hreflang. See [Environment Variables](/recipes/environment-variables/) for per-locale configuration.
- You want to include `lastmod`, `changefreq`, and `priority` metadata derived from content timestamps. See [Cron Jobs](/recipes/cron-jobs/) for scheduled updates.
- You need a sitemap index file that references multiple paginated sitemap files for very large sites. See [Compression Gzip](/recipes/compression-gzip/) for reducing sitemap transfer size.
