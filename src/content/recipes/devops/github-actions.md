...
- Running tests on every pull request to catch regressions before they reach production. See [Unit Testing](/recipes/unit-testing/) for test automation.
- Building and pushing Docker images on release tags for immutable deployments. See [Docker Basics](/recipes/docker-basics/) for image building.
- Deploying to staging or production environments with gated approvals. See [Environment Variables](/recipes/environment-variables/) for managing deployment config.
- Linting and formatting checks before merge to maintain consistent code style. See [Pre-Commit Hooks](/recipes/pre-commit-hooks/) for local code quality.
- Running scheduled tasks (cron-based workflows) such as dependency audits or nightly backups. See [Scheduled Jobs](/recipes/background-jobs/) for cron patterns.
- Generating and publishing documentation automatically when source files change
- Scanning for security vulnerabilities in dependencies on every commit. See [Secret Management](/recipes/secret-management/) for credential security.
