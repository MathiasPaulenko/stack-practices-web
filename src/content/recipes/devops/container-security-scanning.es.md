...
- Workloads de produccion ejecutan aplicaciones containerizadas que deben cumplir compliance de seguridad. Consulta [Docker Basics](/recipes/docker-basics/) para fundamentos de contenedores.
- Las imagenes se construyen desde imagenes base publicas con estado de vulnerabilidad desconocido. Consulta [Docker Compose Local Dev](/recipes/docker-compose-local-dev/) para construcción local de imágenes.
- Necesitas enforcement automatizado bloqueando deployments con CVEs criticas. Consulta [CI/CD Pipeline Setup](/recipes/cicd-pipeline-setup/) para gating de pipelines.
