...
- Production workloads run containerized applications that must meet security compliance. See [Docker Basics](/recipes/docker-basics/) for container fundamentals.
- Images are built from public base images with unknown vulnerability status. See [Docker Compose Local Dev](/recipes/docker-compose-local-dev/) for local image building.
- You need automated enforcement blocking deployments with critical CVEs. See [CI/CD Pipeline Setup](/recipes/cicd-pipeline-setup/) for pipeline gating.
