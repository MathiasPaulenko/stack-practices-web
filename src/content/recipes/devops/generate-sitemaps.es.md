...
- Tu sitio tiene miles de páginas que cambian regularmente y un sitemap estático es inmantenible. Consulta [Background Jobs](/recipes/background-jobs/) para regeneración programada.
- Ejecutas un sitio multi-idioma y necesitas anotaciones `xhtml:link` en sitemaps para hreflang. Consulta [Environment Variables](/recipes/environment-variables/) para configuración por locale.
- Quieres incluir metadatos `lastmod`, `changefreq` y `priority` derivados de timestamps de contenido. Consulta [Cron Jobs](/recipes/cron-jobs/) para actualizaciones programadas.
- Necesitas un archivo de índice de sitemap que referencie múltiples archivos de sitemap paginados para sitios muy grandes. Consulta [Compression Gzip](/recipes/compression-gzip/) para reducir tamaño de transferencia de sitemaps.
...
1. **Cachéa sitemaps con un TTL corto.** Regenera cada 1-6 horas en lugar de en cada request:
...
2. **Genera sitemaps en background jobs.** Para sitios grandes, genera sitemaps vía un cron job y sirve el archivo estático:
...
3. **Comprime sitemaps con gzip.** Reduce el tamaño de transferencia en 70-90% para sitemaps grandes:
