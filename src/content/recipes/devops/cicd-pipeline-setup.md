...
- Setting up a new project and want automated testing from day one. See [Unit Testing](/recipes/unit-testing/) for test fundamentals.
- Migrating from manual deployments to automated releases. See [GitHub Actions](/recipes/github-actions/) for workflow automation.
- Adding security scanning, linting, or code quality gates to your workflow. See [Container Security Scanning](/recipes/container-security-scanning/) for CI security gates.
- Building a multi-environment deployment strategy (dev → staging → prod). See [Blue-Green Deployment](/recipes/blue-green-deployment/) for zero-downtime releases.
