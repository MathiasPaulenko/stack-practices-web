...
Immutable infrastructure treats servers and deployments as disposable artifacts that are never modified after creation. Instead of patching running machines, you build new images from versioned definitions and replace old instances entirely. This eliminates configuration drift, makes rollbacks trivial, and ensures every environment — from development to production — runs identical software stacks.
...
- **Scan images before deploy**: Trivy, Clair, or Snyk in CI pipeline
...
3. **Storing secrets in images**: Bake credentials into AMIs and containers create permanent exposure
...
A: For stateless app servers, yes. For databases, use immutable configuration + persistent data volumes, not immutable data.
