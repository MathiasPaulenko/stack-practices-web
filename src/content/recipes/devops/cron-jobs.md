...
- Running periodic tasks on a server (backups, cleanups, reports). See [Background Jobs](/recipes/background-jobs/) for task queue patterns.
- Scheduling background jobs within an application. See [Scheduled Jobs](/recipes/background-jobs/) for serverless cron.
- Replacing manual processes with automated scripts. See [Bash Scripting Automation](/recipes/bash-scripting-automation/) for script automation.
- Coordinating distributed job execution. See [RabbitMQ Task Queue](/recipes/rabbitmq-task-queue/) for distributed task coordination.
