...
- **Crashes de Prometheus server**: cuando Prometheus crashea, monitoring se pierde. Corre Prometheus en HA setup con dos instances. Usa external storage para long-term data. Monitorea Prometheus health con meta-monitoring setup. Alerta en Prometheus being down. Testea recovery procedures. Documenta runbooks para Prometheus failures. Usa systemd o Kubernetes para auto-restart
- **Fallos de Alertmanager**: cuando Alertmanager cae, alerts no se deliver. Corre multiples Alertmanager instances en HA mode. Usa cluster configuration para deduplication. Monitorea Alertmanager health. Alerta en Alertmanager being down. Testea alert delivery end-to-end. Documenta Alertmanager runbooks. Usa notification redundancy (email + Slack + PagerDuty)
- **Fallos de service discovery**: cuando service discovery falla, targets no se scrapean. Usa multiples service discovery mechanisms. Monitorea service discovery health. Alerta en target count drops. Testea service discovery failover. Documenta service discovery configuration. Usa static fallbacks para critical services. Monitorea target refresh rate
- **Errores de rule evaluation**: PromQL invalido causa rule evaluation failures. Valida rules antes de deployment. Usa promtool para rule testing. Monitorea rule evaluation errors. Alerta en evaluation failures. Usa version control para rules. Documenta rule testing procedures. Revisa rules regularmente. Usa unit tests para complex rules
- **Fallos de remote write**: cuando remote write falla, data no se envia a long-term storage. Usa retry con exponential backoff. Monitorea remote write error rate. Alerta en remote write queue buildup. Implementa local buffering durante outages. Testea remote write failover. Documenta remote write configuration. Usa multiples remote write endpoints

## Performance y Escalabilidad

- **Capacity planning de Prometheus**: dimensiona Prometheus basado en series count e ingestion rate. Estima 1 million series por Prometheus instance. Monitorea memory usage. Alerta en memory pressure. Usa SSD storage para mejor performance. Monitorea disk I/O. Documenta capacity planning guidelines. Planifica 6-month growth. Scalea verticalmente primero, luego usa federation
- **Query performance**: slow queries impactan dashboards y alerts. Usa recording rules para frequent queries. Evita high-cardinality queries. Usa rate() e increase() eficientemente. Limita query time range. Monitorea query duration. Alerta en slow queries. Usa query caching. Documenta query optimization patterns. Profilea slow queries con pprof
- **Optimizacion de storage**: optimiza storage usage. Usa retention period de 15-30 dias. Usa downsampling para long-term data. Monitorea storage growth. Alerta en storage capacity. Comprime old blocks. Usa WAL checkpointing. Documenta storage configuration. Testea storage recovery. Usa thanos o cortex para long-term storage
- **Optimizacion de red**: reduce network overhead. Usa local Prometheus instances por datacenter. Usa federation para global views. Comprime remote write data. Monitorea network latency. Alerta en network issues. Usa gRPC para remote write. Documenta network topology. Testea network failover. Usa private networks para monitoring traffic
