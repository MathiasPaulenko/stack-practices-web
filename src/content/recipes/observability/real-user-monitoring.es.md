...
- **Fallos de carga de RUM script**: cuando el RUM script falla en cargar, user data se pierde. Usa async script loading. Implementa error boundaries alrededor de RUM code. Monitorea script load success rate. Alerta en load failure rate. Usa CDN para script delivery. Testea script loading en diferentes browsers. Documenta fallback behavior. Usa feature detection para browser compatibility
- **Fallos de Beacon API**: cuando sendBeacon falla, eventos se pierden durante page unload. Implementa fallback a fetch con keepalive. Queuea eventos en localStorage. Retry en next page load. Monitorea beacon success rate. Alerta en beacon failures. Testea en diferentes browsers. Documenta beacon limitations. Usa navigator.sendBeacon para reliability
- **Data loss en high traffic**: bajo extreme traffic, RUM events pueden dropearse. Usa sampling para high-traffic sites. Setea sample rate basado en traffic volume. Prioriza error events sobre navigation events. Monitorea event drop rate. Alerta en drop rate excediendo threshold. Usa server-side sampling. Documenta sampling strategy. Revisa sample rate mensualmente
- **Fallos de session replay**: session replay puede fallar en complex SPAs. Testea replay en diferentes page types. Monitorea replay capture rate. Alerta en replay failures. Usa selective replay capture para complex pages. Documenta replay limitations. Usa privacy-conscious replay settings. Testea replay en mobile devices. Revisa replay quality regularmente
- **Compliance de privacidad**: RUM collecta user data que puede requerir consent. Implementa consent management. Anonimiza IP addresses. Maskear sensitive form fields. Usa cookieless tracking donde sea posible. Documenta data collection practices. Comply con GDPR, CCPA. Provee opt-out mechanism. Revisa privacy settings regularmente. Audita data retention

## Performance y Escalabilidad

- **Gestion de RUM data volume**: RUM genera large data volumes. Setea retention period a 30 dias. Usa sampling para high-traffic sites. Comprime event payloads. Monitorea data volume. Alerta en volume spikes. Usa server-side aggregation para metrics. Documenta data volume trends. Planifica capacity basado en traffic. Archiva old data a cold storage
- **Dashboard performance**: RUM dashboards pueden ser slow con large datasets. Usa pre-aggregated metrics. Limita dashboard time range. Usa cached results. Monitorea dashboard load time. Optimiza slow queries. Usa sampling para exploration. Documenta dashboard best practices. Usa real-time vs historical views. Revisa dashboard usage patterns
- **Scaling de event pipeline**: scalea event ingestion basado en traffic. Usa horizontal scaling para ingestion. Monitorea ingestion rate. Setea min/max nodes. Usa load balancers. Testea ingestion bajo peak load. Documenta capacity planning. Alerta en ingestion queue depth. Usa autoscaling basado en traffic. Usa queue-based ingestion para buffering
- **Impacto en client-side performance**: RUM scripts no deben impactar page performance. Usa async loading. Minimiza script size. Usa web workers para processing. Monitorea script execution time. Alerta en script overhead. Usa requestIdleCallback para non-critical events. Documenta performance impact. Testea en low-end devices. Revisa script overhead regularmente
