...
- Users need to download reports or filtered data from a web app. See [Import CSV Excel](/recipes/import-csv-excel/) for the reverse workflow.
- Migrating data between systems requires an intermediate file format. See [Parse JSON](/recipes/parse-json/) for structured data exchange.
- Building an admin panel with bulk export functionality. See [Background Jobs](/recipes/background-jobs/) for async report generation.
- Processing data for external tools (spreadsheets, BI tools). See [Stream Processing](/recipes/stream-processing/) for large dataset pipelines.
