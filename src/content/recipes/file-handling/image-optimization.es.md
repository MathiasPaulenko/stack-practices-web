...
- Los usuarios suban fotos que necesiten redimensionarse antes de almacenarse. Consulta [File Upload Validation](/recipes/file-upload-validation/) para manejo seguro de subidas.
- Necesites generar múltiples tamaños de imagen para layouts responsivos. Consulta [SPA Code Splitting](/recipes/spa-code-splitting-lazy/) para entrega de assets responsive.
- Tu informe de Core Web Vitales señale imágenes para optimización. Consulta [Compression Gzip](/recipes/compression-gzip/) para reducción adicional de payload.
- Quieras convertir formatos legacy (JPEG, PNG) a WebP/AVIF. Consulta [Stream Processing](/recipes/stream-processing/) para conversión de formato por lotes.
