...
- Construyas una app web que acepte imágenes, documentos o media generados por usuarios. Consulta [Optimización de Imágenes](/recipes/image-optimization/) para procesamiento post-subida.
- Implementes un CMS, foro o SaaS con soporte de adjuntos. Consulta [Exportar CSV Excel](/recipes/export-csv-excel/) para capacidades de exportación de datos.
- Necesites cumplir con estándares de seguridad (PCI-DSS, SOC 2). Consulta [Gestión de Secretos](/recipes/secret-management/) para almacenamiento seguro de credenciales.
- Proceses archivos de fuentes no confiables (formularios públicos, APIs). Consulta [Input Validation](/recipes/input-validation/) para manejo de entrada no confiable.
