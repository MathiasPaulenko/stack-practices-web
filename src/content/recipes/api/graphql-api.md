...
- You need to aggregate data from multiple microservices. See [gRPC API](/recipes/grpc-api/) for service-to-service communication.
...
- **Implement pagination**: Use [cursor-based pagination](/recipes/cursor-pagination-postgresql/) for large lists (Relay Connection spec)
...
- **Missing error handling**: GraphQL returns 200 OK even with errors — always check the `errors` array.  See [Error Handling](/recipes/handle-errors/) for patterns.
...
- **Related guides**: explore the api and graphql guides for deeper coverage.
...
- **Apply implement a graphql api** when you need a practical solution for your use case.
...
A: Not necessarily. GraphQL shines when clients need flexibility. If your API has simple, stable consumers, [REST](/recipes/call-rest-api/) may be simpler and more cacheable.
...
- **Version your schema, not your endpoints**: GraphQL has one endpoint.  Add fields with deprecation markers instead of creating new queries.
...
- **Query parsing overhead**: each GraphQL request parses and validates the query against the schema.  At 10K requests/second, parsing adds 5-15ms per request.
...
- **Subscription scalability**: WebSocket subscriptions maintain persistent connections.  At 10K concurrent subscriptions, each consuming 50KB of memory, you need 500MB just for connections.
...
- **Test resolver logic in isolation**: call resolvers directly with mock context and arguments.  Verify return values, error handling, and N+1 query prevention.
- **Test schema with introspection queries**: run introspection queries to verify the schema exposes only intended types and fields.
- **Test query complexity limits**: send deeply nested queries and queries with high field counts.  Verify the complexity analyzer rejects them with a clear error message.
...
- **N+1 query problem**: resolvers that fetch related data individually cause N+1 database queries.  A query for 100 users with their posts triggers 1 + 100 = 101 queries.
- **Exposing the entire schema in production**: introspection allows clients to discover all types and fields.  Disable introspection in production to prevent attackers from mapping your API surface.
...
- **Monitor resolver execution time**: track p50, p95, and p99 latency per resolver field.  Slow resolvers (p95 >100ms) are the primary bottleneck.
...
- **Introspection-based reconnaissance**: in production, disable introspection to prevent attackers from discovering all types, fields, and mutations.
