...
- Aplicas autenticación antes de que los route handlers se ejecuten. Consulta [JWT Authentication](/recipes/jwt-authentication/) para patrones de auth.
- Logueas todos los requests entrantes y tiempos de respuesta. Consulta [Logging](/recipes/logging/) para logging estructurado.
- Agregas headers CORS o de seguridad a cada response. Consulta [Manejo de CORS](/recipes/handle-cors/) para configuración de CORS.
- Validas cuerpos de request o parámetros de query
- Implementas rate limiting o throttling de requests. Consulta [Rate Limiting con Redis](/recipes/api-rate-limiting-redis/) para rate limiting basado en Redis.
...
- **preHandle / before**: Puede bloquear requests (auth, validación).
...
- **Error middleware**: Captura excepciones no atrapadas.   Debe registrarse al final del stack.   Consulta [Manejo de Errores](/recipes/handle-errors/) para patrones de error.

## Lo que funciona

- **El orden importa**: Registra middleware en la secuencia correcta (ej.
...
- **Guías relacionadas**: explora las guías de api y authentication para profundizar.
...
- **Aplica middleware** cuando necesites una solución práctica para tu caso de uso.
