...
- **Deprecation** should be signaled with `Sunset` headers and changelog documentation, giving clients a clear migration window. See [API Deprecation Notice Template](/docs/api-deprecation-notice-template/) for deprecation communication.
...
1. Bumping the version for every minor change, fragmenting the client ecosystem. See [Input Validation](/recipes/input-validation/) for safe additive changes.
...
- **Apply api versioning** when you need a practical solution for your use case.
...
Only for breaking changes: removed fields, renamed resources, changed auth requirements, or altered behavior that existing clients depend on. [Additive changes](/recipes/input-validation/) (new optional fields, new endpoints) do not require a version bump.
