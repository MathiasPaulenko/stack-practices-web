...
- Las [consultas a base de datos](/recipes/postgres-query-optimization/) son lentas y devuelven los mismos resultados frecuentemente
- Necesitas reducir carga en [APIs](/recipes/call-rest-api/) o bases de datos downstream
- Datos de sesión, perfiles de usuario o configuración necesitan acceso rápido de lectura
- Se requieren leaderboards en tiempo real, [rate limiting](/recipes/rate-limiting/) o locks temporales
...
- **Maneja cache misses elegantemente**: Los fallos de caché deben degradar a la base de datos, nunca crashear la app. Usa [circuit breakers](/patterns/circuit-breaker-pattern/) para conexiones Redis.
