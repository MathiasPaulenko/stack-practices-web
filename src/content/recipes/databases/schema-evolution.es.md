...
Los schemas de base de datos deben evolucionar a medida que las aplicaciones crecen, pero los cambios de schema son una causa principal de [outages en producción](/guides/on-call-incident-response-guide/). El patrón expand-contract, online DDL y migraciones backward-compatible permiten a los equipos agregar capacidades sin downtime. Este recurso cubre técnicas prácticas para evolucionar schemas en PostgreSQL, MySQL y bases de datos distribuidas manteniendo la integridad de datos y disponibilidad de aplicaciones.
...
4. **Ignorar timeouts de lock**: `statement_timeout` de PostgreSQL aborta migraciones largas impredeciblemente. Consulta [connection pooling](/recipes/connection-pooling/).
