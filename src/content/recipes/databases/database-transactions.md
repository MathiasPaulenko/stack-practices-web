...
- Transferring money between accounts. See [Money and Currency](/recipes/money-currency/) for exact decimal arithmetic.
- Updating inventory after a purchase. See [Batch Processing](/recipes/batch-processing-patterns/) for bulk operations.
- Creating related records across multiple tables
- Ensuring read consistency for reporting queries
- Preventing [race conditions](/recipes/race-condition-prevention/) in concurrent writes
...
- **Use optimistic locking** for high-contention data (version columns). See [Optimistic Locking](/recipes/optimistic-locking/) for version-based concurrency.
...
- Not handling [deadlock exceptions](/recipes/database-deadlocks-retries/) (error code 40P01 in PostgreSQL)
