...
Database replication copies data from a primary database to one or more replicas. This enables read scaling, high availability, and [disaster recovery](/guides/on-call-incident-response-guide/). Whether using streaming replication in PostgreSQL, binary log replication in MySQL, or native replica sets in MongoDB, understanding replication lag, failover, and consistency trade-offs is essential for building resilient data layers.
...
- [Backups](/guides/on-call-incident-response-guide/) must not impact primary database performance
...
- **Use connection pooling**: [PgBouncer](/recipes/connection-pooling/) or ProxySQL manages primary/replica routing
...
A: Yes. `pg_basebackup` from a replica offloads the primary. Ensure the replica is caught up first. See our [disaster recovery plan template](/guides/on-call-incident-response-guide/).
