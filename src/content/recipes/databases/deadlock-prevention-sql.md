...
- Concurrent [transactions](/recipes/database-transactions/) fail intermittently with deadlock errors
- [Batch operations](/recipes/batch-processing-patterns/) and user-facing transactions compete for the same rows
...
- **[Optimistic locking](/recipes/optimistic-locking/)** uses versioning instead of database locks, reducing contention
...
- Use `SELECT FOR UPDATE SKIP LOCKED` for queue-like workloads. See [Locks and Mutexes](/recipes/locks-and-mutexes/) for coordination.
...
A: A [race condition](/recipes/race-condition-prevention/) is a timing-dependent bug in correctness. A deadlock is a blocking condition where transactions wait indefinitely for each other.
