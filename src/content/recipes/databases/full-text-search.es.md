...
- Los usuarios necesiten buscar artículos, productos o documentos con tolerancia a errores tipográficos. Consulta [Parse JSON](/recipes/parse-json/) para indexación de documentos.
- Tu app requiera resultados clasificados, búsqueda facetada o resaltado
- Las consultas `LIKE` sean demasiado lentas en tablas con >100k filas. Consulta [Query Optimization](/recipes/postgres-query-optimization/) para ajuste de índices.
- Quieras comportamiento de búsqueda mientras escribes (autocompletado). Consulta [Input Validation](/recipes/input-validation/) para sanitización de consultas.
...
- **Usa índices GIN en PostgreSQL**: Las consultas `to_tsvector` sin índice son escaneos completos de tabla. Consulta [Query Optimization](/recipes/postgres-query-optimization/) para indexación.
...
- **Almacenar todos los datos en el motor de búsqueda**: Usa IDs de búsqueda para obtener registros completos de la base de datos. Consulta [Data Validation](/recipes/data-validation/) para integridad de datos.
