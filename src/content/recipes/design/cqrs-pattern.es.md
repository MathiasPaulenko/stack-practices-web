...
- Trabajando con sistemas event-sourced donde el estado se deriva de una secuencia de eventos. Consulta [Event Sourcing](/recipes/event-sourcing-relational/) para patrones de event store.
...
R: No. CQRS separa lecturas y escrituras. [Event sourcing](/recipes/event-sourcing-relational/) almacena estado como eventos. Se usan frecuentemente juntos porque event sourcing produce naturalmente eventos que pueden poblar read models. Pero puedes usar CQRS sin event sourcing (ej. dual databases) y event sourcing sin CQRS (modelo único reconstruido desde eventos).
