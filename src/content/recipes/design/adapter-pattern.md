...
- **Test adapters with contract tests**: write tests that verify the adapter satisfies the target interface, not tests that verify the third-party SDK. Use mocks of the adaptee to test the adapter in isolation. See [Input Validation](/recipes/input-validation/) for boundary contracts.
