...
- **Make singletons stateless or immutable**: a mutable singleton is global state, and global state is the enemy of testing and concurrency. See [Race Condition Prevention](/recipes/race-condition-prevention/) for concurrent safety. If the singleton must hold state, make it thread-safe (use locks or atomic operations) and document thread-safety guarantees.
