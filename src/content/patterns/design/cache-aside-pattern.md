...
- Examples: user profiles, product catalogs, [configuration data](/recipes/parse-json/), reference data
...
|---------|-------------|----------|
| **[Lazy Loading](/recipes/lazy-loading/)** | Cache miss triggers load | Most common; prevents unnecessary cache fills |
| **[Write-Through](/recipes/cache-invalidation/)** | Writes update cache and DB simultaneously | Strong consistency required |
...
- **Use a [circuit breaker](/patterns/circuit-breaker-pattern/)** around cache failures — if Redis is down, fall back directly to DB
...
- **Apply cache-aside pattern** when you need a practical solution for your use case.
...
A: Use a [mutex](/recipes/rate-limiting/) or lock per key so only one request loads from DB while others wait. Alternatively, see [caching strategies](/recipes/caching-strategies/) for more patterns. Alternatively, use a probabilistic early expiration (e.g., refresh the key before TTL expires with some probability).
