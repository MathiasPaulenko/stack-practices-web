...
The [Dependency Injection](/patterns/dependency-injection-typescript/) Pattern is an architectural pattern where dependencies are supplied to a class from the outside rather than being created internally. This inverts control: the class declares what it needs, and an external mechanism provides it. The result is loosely coupled, highly testable code.
...
| **[DI Container](/patterns/dependency-injection-typescript/)** | Framework resolves and injects dependencies automatically | Large applications (Spring, Angular, .NET Core) |
...
A: No. For small projects, manual constructor injection is sufficient. See [DI Container in TypeScript](/patterns/dependency-injection-typescript/) for a lightweight implementation. DI frameworks like Spring, Angular's injector, or InversifyJS shine in large applications with many interdependent services.

**Q: How does DI help with testing?**
A: By depending on abstractions (interfaces), you can inject mock or stub implementations during tests. See [unit testing](/recipes/unit-testing/) for testing patterns. This isolates the class under test from its real collaborators.
