...
A: Related but different. [Observer](/patterns/observer-pattern/) is a one-to-many dependency where subjects notify observers. Mediator centralizes many-to-many communication. An event bus can serve as both.

**Q: What is the difference between Mediator and Facade?**
A: [Facade](/patterns/adapter-pattern/) provides a simplified interface to a subsystem. Mediator coordinates communication between peer objects. Facade is about simplifying access; Mediator is about decoupling peers.
