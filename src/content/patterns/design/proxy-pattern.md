...
A: Both wrap objects and add behavior. Proxy controls access to the wrapped object (often for structural reasons like lazy loading or remote access). See [Caching Proxy](/patterns/proxy-pattern-caching/) for a concrete example. Decorator adds responsibilities dynamically, usually for functional enhancement. The intent differs even if the structure looks similar.
...
A: Use a [Factory](/patterns/factory-pattern/) when you want to control which class is instantiated. Use a Proxy when you want to control access to an already-instantiated object or delay its creation.
