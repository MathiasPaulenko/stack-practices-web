...
- Ejemplos: perfiles de usuario, catálogos de productos, [datos de configuración](/recipes/parse-json/), datos de referencia
...
|----------|-------------|-------------|
| **[Lazy Loading](/recipes/lazy-loading/)** | El cache miss dispara la carga | Más común; previene llenados innecesarios del caché |
| **[Write-Through](/recipes/cache-invalidation/)** | Las escrituras actualizan caché y DB simultáneamente | Consistencia fuerte requerida |
...
- **Usa un [circuit breaker](/patterns/circuit-breaker-pattern/)** alrededor de fallas de caché — si Redis está caído, recurre directamente a la DB
...
R: Usa un [mutex](/recipes/rate-limiting/) o lock por clave para que solo una petición cargue desde la DB mientras otras esperan. Alternativamente, consulta [estrategias de caché](/recipes/caching-strategies/) para más patrones, o usa expiración temprana probabilística (ej. refresca la clave antes de que expire el TTL con cierta probabilidad).
