...
- [CQRS](/patterns/cqrs-pattern/) is in use, and read models can be built from event projections
- You need to compensate for failures by replaying or reversing events (see [Saga](/patterns/saga-pattern/))
...
- **Events**: Immutable records describing what happened (e. g.
...
- **Apply event sourcing pattern** when you need a practical solution for your use case.
...
A: After every N events, save the computed entity state. On rehydration, load the latest snapshot and replay only events after it. This keeps replay time constant. See [CQRS](/patterns/cqrs-pattern/) for read model patterns that work well with Event Sourcing.
