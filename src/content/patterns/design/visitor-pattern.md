...
- You need to perform operations on all elements of a complex object structure. See [Composite Pattern](/patterns/composite-pattern/) for tree structures.
- The object structure is stable but operations on it change frequently. See [Strategy Pattern](/patterns/strategy-pattern/) for interchangeable algorithms.
- You want to avoid polluting element classes with unrelated operations. See [Single Responsibility Principle](/patterns/solid-principles-typescript/) for focused classes.
...
- **Use alongside [Composite](/patterns/composite-pattern/)** for traversing tree structures
...
- **Related guides**: explore the visitor and pattern guides for deeper coverage.
...
- **Apply visitor pattern** when you need a practical solution for your use case.
...
A: If the operation is specific to a client use case (e.g., PDF export) and not intrinsic to the element, adding it directly violates the [Single Responsibility Principle](/patterns/solid-principles-typescript/). Visitor keeps element classes focused.
