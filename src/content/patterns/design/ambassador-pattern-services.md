...
---
The Ambassador pattern creates a local helper instance that acts on behalf of a remote service. It handles network concerns like retries, timeouts, circuit breaking, and logging, keeping the client code clean and the remote service interface simple. This pattern is common in [microservices](/guides/microservices-architecture-guide/) and containerized deployments.
...
- Combine with a [service mesh](/guides/microservices-architecture-guide/) (Istio, Linkerd) for cluster-wide policy enforcement
...
- **Apply ambassador pattern for resilient remote service access** when you need a practical solution for your use case.
...
A: [Proxy](/patterns/proxy-pattern/) controls access to a single object. Ambassador specifically handles remote service resilience and is usually deployed as a local process or library.
