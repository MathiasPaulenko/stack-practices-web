...
El [Patrón Saga](/patterns/saga-pattern/) gestiona transacciones distribuidas a través de múltiples servicios rompiendo una transacción de larga duración en una secuencia de transacciones locales. Para coreografía impulsada por eventos, consulta [Event Sourcing](/patterns/event-sourcing-pattern/). Cada transacción local actualiza un único servicio y publica un evento o mensaje para disparar el siguiente paso. Si un paso falla, la saga ejecuta **transacciones compensatorias** para deshacer los cambios realizados por los pasos anteriores.
...
- **[Reintentos](/patterns/retry-pattern/)**: Reintenta fallas transitorias dentro de un paso antes de declarar fallo
