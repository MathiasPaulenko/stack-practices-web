...
The [State](/patterns/state-pattern/) Pattern is a behavioral design pattern that lets an object alter its behavior when its internal state changes. Instead of large switch statements, each state is encapsulated in its own class with state-specific behavior.
...
| **Pushdown Automaton** | Stack-based state history | Undo/reversible state transitions. See [Memento](/patterns/memento-pattern/) |
...
- **Apply state pattern** when you need a practical solution for your use case.
...
A: Both encapsulate behavior in interchangeable objects. State is about changing behavior based on internal state transitions. [Strategy](/patterns/strategy-pattern/) is about selecting an algorithm from the outside. The intent differs: State manages lifecycle, Strategy provides options.
