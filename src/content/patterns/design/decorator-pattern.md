...
It is widely used in I/O streams (Java), [middleware pipelines](/patterns/decorator-pattern-pipeline/) (Express.js), and Python's `@decorator` syntax.
...
| **[Middleware pipeline](/patterns/decorator-pattern-pipeline/)** | Web frameworks (Express, Koa) | Great for request/response processing |
...
A: Decorator adds responsibilities dynamically. [Proxy](/patterns/proxy-pattern/) controls access to an object (lazy initialization, access control, logging). They have similar structure but different intent.

**Q: Can decorators be removed at runtime?**
A: Not easily in most implementations. If you need add/remove flexibility, consider the [Chain of Responsibility](/patterns/chain-of-responsibility-pattern/) pattern instead.
