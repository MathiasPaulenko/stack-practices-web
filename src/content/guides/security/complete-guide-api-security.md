...
description: "Secure your APIs end-to-end. Covers rate limiting, authentication, input validation, CORS, SQL injection prevention, API gateway patterns, request size limits, pagination security, mass assignment, versioning, audit logging, and API security testing with practical code examples."
metaDescription: "Secure APIs end-to-end. Covers rate limiting, auth, input validation, CORS, SQL injection, API gateway, mass assignment, audit logging, testing."
difficulty: advanced
topics:
  - security
  - api
...
APIs are the primary attack surface for modern applications. Every endpoint is a potential entry point for attackers. This guide walks through rate limiting, authentication, input validation, CORS, injection prevention, API gateway patterns, mass assignment protection, audit logging, and security testing with production-ready code examples.
...
# Usage: 1000 requests per hour per API key
...
def validate_file_upload(file_bytes: bytes, filename: str) -> str:
    ...
    # Detect actual MIME type from content (not from extension)
...
# This is a security vulnerability — browsers will reject it
```
...
# SAFE: SQLAlchemy with explicit parameters
...
# SAFE: Input sanitization for dynamic queries
import re

def safe_order_by(column: str, allowed_columns: set) -> str:
    """Validate column name for ORDER BY clause."""
...
async def update_user(user_id: int, body: dict = Body(...)):
    ...
    for key, value in body.items():
        setattr(user, key, value)  # Attacker can set role=admin!
    db.commit()
    return user

# SAFE: Explicit field mapping with Pydantic
class UpdateUserRequest(BaseModel):
    username: Optional[constr(min_length=3, max_length=50)] = None
    email: Optional[constr(max_length=255)] = None
    bio: Optional[constr(max_length=500)] = None
    # Note: 'role', 'is_admin', 'password_hash' are NOT included
...
class APIGatewayMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        # 7. Remove sensitive headers from response
...
async def list_users(page: int = 1, size: int = 1000):
    # Attacker can request size=1000000 and overwhelm the server
    offset = (page - 1) * size
...
# SAFE: Cursor-based pagination (better for large datasets)
...
async def list_users_cursor(
    ...
    users = query.limit(size + 1).all()  # Fetch one extra to check has_more
...
Input Validation:
  [ ] All inputs validated with schema (Pydantic, Zod, Joi)
...
Rate Limiting:
  ...
  [ ] Tiered limits based on API plan
  [ ] 429 response with Retry-After header
  [ ] Distributed rate limiting (Redis) for multi-instance

CORS:
  [ ] Specific origins allowed (no wildcard with credentials)
...
### How do I choose between API keys and JWT for authentication?

Use API keys for server-to-server communication and long-lived access (service accounts, CI/CD pipelines). Use JWT for user-facing applications where tokens expire and can be refreshed. API keys are simpler but cannot encode claims or expire automatically. JWTs carry user context (roles, permissions) and expire, but require refresh token management. For public APIs, offer both: API keys for developers and OAuth2/JWT for end-user auth.

### What is the best rate limiting algorithm?

Token bucket is the most flexible — it allows bursts up to the bucket capacity while maintaining an average rate. Sliding window provides the most accurate counting but requires more memory. Fixed window is simplest but allows bursts at window boundaries. Use token bucket for user-facing APIs (allows short bursts), sliding window for strict limits (payment APIs), and Redis for distributed rate limiting across multiple instances.

### How do I prevent mass assignment vulnerabilities?

Never use `setattr` or `Object.assign` with raw user input. Define explicit request schemas (Pydantic, DTOs) that only include fields the user is allowed to update. Exclude sensitive fields like `role`, `is_admin`, `password_hash`, `balance` from update schemas. Use `dict(exclude_unset=True)` to only update fields that were explicitly provided. Test mass assignment by sending extra fields in requests and verifying they are ignored.

### Should I use cursor-based or offset-based pagination?

Use cursor-based pagination for large datasets and real-time feeds. It is faster (no COUNT query) and avoids skipping items when new data is inserted. Use offset-based pagination for small datasets where you need total page count and random page access. Always enforce a maximum page size (e.g., 100 items) regardless of the pagination method to prevent denial of service.

### How do I secure file uploads?

Validate the actual MIME type using content inspection (python-magic, file command), not the file extension. Limit file size (e.g., 10MB). Store uploads outside the web root or in object storage (S3, GCS). Generate new filenames — never use the user-supplied filename. Scan for embedded scripts in images. Use a dedicated upload service if processing files (virus scan, image resize). Set `Content-Disposition: attachment` on download to prevent inline execution.
