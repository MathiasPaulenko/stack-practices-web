...
- For alternatives, see [Complete Guide to Terraform Modules](/guides/complete-guide-terraform-modules/).
...
### Scenario: Modular Terraform for Production
...
Use a remote backend (S3 + DynamoDB for locking). S3 stores the state file. DynamoDB prevents concurrent writes. Configure `encrypt = true` on the backend. Never commit the .tfstate file to the repo. Use `terraform state pull` and `terraform state push` with caution. For large teams, use Terraform Cloud or Atlantis to apply changes via PRs.
