...
- **Monitorea lag de replicación** — DDL grande puede bloquear replicación; pausa si el lag excede umbrales. Consulta la [Guía de Optimización de SQL](/guides/sql-performance-tuning-guide/) para estrategias de monitoreo.
...
- Correr migraciones no testeadas en producción — testea en una copia con tamaño de datos realista. Documenta tu esquema con la [Plantilla de Documentación de Esquema de BD](/docs/database-schema-documentation-template/).
...
- **Guías relacionadas**: explora las guías de database y rollback para profundizar.
...
- **Aplica plantilla de runbook de migración de base de datos** cuando necesites una solución práctica para tu caso de uso.
...
Usa una herramienta (Flyway, Liquibase, Django migrations, Rails migrations). Consulta la [Guía de Optimización de SQL](/guides/sql-performance-tuning-guide/) y la [Guía de Sharding de BD](/guides/database-sharding-partitioning-guide/) para pautas relacionadas. Las herramientas trackean migraciones aplicadas, enforcean ordenamiento, y proveen hooks de rollback. Los scripts SQL raw requieren trackeo manual y son propensos a errores.

### ¿Cómo manejo una migración fallida en producción?

Detente inmediatamente. No apliques migraciones subsiguientes. Evalúa si hacer rollback o fix forward. Rollback si la integridad de datos está en riesgo. Fix forward si el fix es un script pequeño y bien entendido. Siempre ten el script de rollback listo antes de empezar. Para planificación más amplia de desastres, consulta la [Plantilla de Plan de Recuperación ante Desastres](/docs/disaster-recovery-plan-template/).
