...
const TAG_MERGE_MAP = {
  ...
  // --- Round 5: aggressive consolidation ---
  testing: ['a-b-testing', 'ab-testing', 'e2e-testing', 'component-testing', 'manual-testing', 'penetration-testing', 'mutation-testing'],
  performance: ['performance-budget', 'performance-testing', 'read-performance'],
  security: ['ci-cd-security', 'content-security-policy', 'endpoint-security', 'network-security'],
  cloud: ['cloud-portability', 'cloud-run', 'cloud-sql', 'google-cloud', 'google-cloud-storage', 'hybrid-cloud', 'multi-cloud'],
  data: ['data-access', 'data-access-layer', 'data-access-object', 'data-as-a-product', 'data-binding', 'data-breach', 'data-fetching', 'data-flow-diagrams', 'data-gravity', 'data-lake', 'data-leakage', 'data-mapper', 'data-mesh', 'data-preparation', 'data-protection', 'data-redundancy', 'data-retention', 'data-structures', 'data-transformation', 'data-transformations', 'decentralized-data', 'connected-data'],
  database: ['database-per-service', 'database-tuning', 'database-tradeoffs', 'graph-database'],
  documentation: ['api-documentation'],
  report: ['bug-report'],
  architecture: ['onion-architecture', 'architecture-decision-record'],
  incident: ['incident-analysis', 'incident-communication', 'incident-review'],
  kafka: ['kafka-connect', 'kafka-streams'],
  lambda: ['lambda-authorizer', 'lambda-layers'],
  react: ['react-19', 'react-compiler', 'react-hook-form', 'react-query', 'react-window'],
  graphql: ['graphql-errors', 'graphql-federation', 'graphql-interfaces', 'graphql-performance'],
  html: ['html-parsing', 'html-templates'],
  http: ['http-status-codes'],
  infrastructure: ['immutable-infrastructure'],
  metrics: ['metrics-collection', 'llm-metrics'],
  monitoring: ['health-endpoint-monitoring'],
  pipeline: ['pipeline-hardening'],
  production: ['production-readiness'],
  release: ['progressive-release'],
  rollout: ['gradual-rollout'],
  queue: ['queue-design'],
  evaluation: ['rag-evaluation', 'lazy-evaluation'],
  analytics: ['real-time-analytics', 'analytics-engineering'],
  cache: ['embedding-cache', 'lru-cache', 'multi-level-cache'],
  configuration: ['external-configuration-store'],
  consumer: ['producer-consumer', 'idempotent-consumer'],
  deduplication: ['message-deduplication'],
  design: ['privacy-by-design', 'object-oriented-design'],
  development: ['local-development'],
  distributed: ['distributed-databases', 'distributed-transactions'],
  identity: ['federated-identity', 'identity-governance', 'identity-map', 'identity-provider', 'identity-verification'],
  injection: ['fault-injection'],
  management: ['job-management', 'knowledge-management', 'memory-management', 'project-management'],
  optimization: ['image-optimization', 'memory-optimization'],
  owasp: ['owasp-llm'],
  partitioning: ['horizontal-partitioning'],
  policy: ['policy-engine'],
  privacy: ['privacy-by-design'],
  prometheus: ['prometheus-client'],
  routing: ['gateway-routing', 'model-routing'],
  safety: ['memory-safety'],
  sanitization: ['input-sanitization'],
  search: ['hybrid-search'],
  secrets: ['external-secrets'],
  state: ['finite-state-machine'],
  // --- Round 5b: remaining aggressive opportunities ---
  edge: ['edge-computing'],
  entity: ['entity-component-system'],
  files: ['env-files', 'temp-files'],
  encryption: ['envelope-encryption'],
  eviction: ['eviction-policy'],
  composition: ['function-composition'],
  operations: ['hanging-operations'],
  review: ['human-review'],
  observer: ['intersection-observer', 'observer-pattern'],
  legacy: ['legacy-code'],
  guardrails: ['llm-guardrails'],
  rotation: ['log-rotation'],
  replication: ['logical-replication'],
  queue: ['priority-queue', 'task-queue'],
  template: ['prompt-template', 'template-method'],
  concurrency: ['provisioned-concurrency'],
  proxy: ['rds-proxy', 'reverse-proxy'],
  reactive: ['reactive-streams'],
  monitoring: ['real-user-monitoring'],
  redis: ['redis-cluster'],
  schema: ['schema-merging', 'schema-review', 'schema-stitching'],
  security: ['row-level-security', 'security-assessment', 'security-controls', 'security-design', 'security-hardening', 'security-playbook', 'security-policy', 'security-review', 'supply-chain-security'],
  serverless: ['serverless-db-pooling'],
  ...
  streaming: ['spark-streaming', 'streaming-replication'],
  management: ['stakeholder-management'],
  state: ['event-carried-state-transfer', 'remote-state', 'state-replication', 'state-snapshot'],
  functions: ['step-functions'],
  strategy: ['strategy-pattern'],
  testing: ['resolver-testing', 'stress-testing'],
  ...
  risk: ['risk-reduction', 'third-party-risk'],
  pip: ['pip-audit'],
// --- Round 6: aggressive domain consolidation ---
  ai: ['langgraph', 'huggingface', 'transformers', 'inference', 'cosine-similarity', 'structured-output', 'chromadb', 'sentence-transformers', 'sentiment-analysis', 'prompt-chaining', 'classification', 'chain-of-thought', 'few-shot'],
  python: ['pydantic', 'nltk', 'vader', 'django', 'httpx', 'cachecontrol', 'pymemcache', 'apscheduler', 'libphonenumber', 'reportlab', 'fpdf', 'xlsx', 'dask', 'openpyxl', 'polars', 'spark', 'pyspark', 'beautifulsoup', 'mypy', 'pip', 'sqlalchemy'],
  javascript: ['fetch', 'socket-io', 'jsonwebtoken', 'passport', 'dom', 'svelte', 'vue', 'astro', 'webpack', 'vite', 'nextjs', 'web-components', 'custom-elements', 'shadow-dom'],
  architecture: ['systems', 'multi-tenancy', 'service-discovery', 'workflow-engine', 'bff', 'facade', 'inbox', 'materialized-view', 'outbox', 'plugin', 'backoff'],
  security: ['argon2', 'password-hashing', 'abac', 'roles', 'permissions', 'xml-signature', 'role', 'dependency-vulnerability', 'penetration-test', 'vulnerability-scan', 'passkeys', 'mfa', 'nonces', 'hashes', 'preflight', 'cross-origin', 'aes-256', 'pkce', 'mitigation', 'doppler', 'sigstore', 'slsa', 'provenance', 'hsts', 'x-frame-options', 'trust-service-criteria', 'hashing', 'digital-signatures', 'aes', 'rsa', 'vulnerability-mitigation', 'defensive-programming', 'threat-modeling', 'stride', 'attack-trees', 'replay-attacks', 'micro-segmentation'],
  git: ['github', 'rebase', 'gitlab'],
  java: ['caffeine', 'loom', 'jdk-21', 'junit'],
  caching: ['etag', 'memcached'],
  csharp: ['dotnet'],
  concurrency: ['task', 'completable-future', 'task-groups', 'threading', 'tokio', 'synchronization', 'coroutines'],
  data: ['diff', 'phone', 'slug', 'url', 'spreadsheet', 'toml', 'transformations', 'spreadsheets', 'qr-code', 'qrcode', 'generation', 'parquet', 'aggregation', 'web-scraping', 'serialization', 'recursive', 'truncation', 'strings', 'uuid', 'trees', 'lineage', 'parser', 'elt', 'delta-lake', 'b-tree'],
  databases: ['ioredis', 'seeding', 'prisma', 'gin', 'tsvector', 'indexes', 'explain', 'ranking', 'crud', 'olap', 'base', 'denormalization', 'faunadb', 'planetscale'],
  infrastructure: ['ansible', 'disk-usage', 'restore', 'pulumi', 'crossplane', 'infraestructura'],
  devops: ['provisioning', 'script', 'kustomize', 'flux'],
  observability: ['fluentd', 'otel', 'alert-fatigue', 'pagerduty', 'opsgenie', 'tempo', 'splunk', 'datadog', 'continuous-improvement'],
  linux: ['grep', 'sed', 'awk'],
  frontend: ['responsive', 'tailwind', 'components', 'grid'],
  messaging: ['celery', 'event-bus', 'sns', 'topics', 'partitions'],
  testing: ['property-based', 'golden-master', 'test', 'dynamic-tests', 'quickcheck', 'plugins', 'test-driven-development', 'red-green-refactor', 'e2e'],
  azure: ['blob', 'active-directory'],
  modules: ['module'],
  api: ['filtering', 'openapi', 'swagger', 'webhook'],
  networking: ['dns', 'mtls'],
  aws: ['iam', 'rds', 'x-ray'],
  agile: ['planning'],
  management: ['prioritization'],
  gcp: ['bigquery', 'gcp-secret-manager'],
  performance: ['tree-shaking', 'bundle-splitting'],
